    ...
    id='work' className='w-full px-[12%] py-10 scroll-m-20'>
        ...
        className='text-center max-w-2xl mx-auto mt-5 mb-12 font-Ovo'>Explore my portfolio to see a collection of creative, user-focused web development projects crafted with modern technologies and innovative design. Each project reflects my commitment to delivering seamless user experiences and functional excellence.</motion.p>
            ...
            {categories.map((category, index)=>(
                ...
                whileInView={{opacity: 1}}
                transition={{delay: 0.7, duration: 0.5}}                 
                key={index} onClick={()=>setSelectedCategory(category)} className={`px-10 sm:py-5 py-3 border ${selectedCategory === category ? 'bg-btnHover dark:bg-btnHover' : ' bg-black'} hover:bg-btnHover transition-all ease-in border-white rounded-full text-white flex items-center gap-2 dark:bg-transparent text-sm sm:text-[16px]`}>{category.toUpperCase()} <span className='sm:block hidden'>PROJECT</span></motion.button>
            ))
            }
            </div>
        <motion.div 
